...
PostFader v0.20.0 is the verified AI copilot for FL Studio. It starts
read-only, never saves the project automatically, and reads supported direct
changes back from FL Studio after the update. When FL Studio exposes weaker
evidence, PostFader labels it as partial or unverified instead of presenting it
as verified.
...
v0.20.0 brings the complete current PostFader control and analysis surface:

- 90 tools and 8 live resources for project inspection and bounded control;
- mixer, Channel Rack, plug-in, pattern, Playlist, history, transport, and
  step-sequencer control;
- Mix Doctor, peak watches, gain-staging plans, reference and masking analysis,
  and reviewable one-shot mix plans;
- processing intents and plug-in profiles that keep parameter discovery tied to
  the evidence FL Studio actually exposes;
- deterministic composition, Type-1 MIDI export with verification, tempo and
  key analysis, and monophonic transcription;
- Piano Roll preparation and transforms through the optional generated script
  workflow;
- guided setup that resolves the local paths and endpoint, deploys the matching
  bridge, generates client configuration, and runs the doctor;
- dedicated Codex packages for Windows and macOS; and
- Windows and macOS qualification of the v0.20 release surface.

The safety model remains part of the product: writes are disabled until the
present user enables them for the current session, Master writes require
explicit permission, and supported setters use later-idle-tick readback. An
ambiguous mutation outcome is never automatically replayed.
