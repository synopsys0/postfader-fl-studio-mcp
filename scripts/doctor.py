...
from fl_studio_mcp.bridge_stamp import BridgeStampError, stamp_bridge_source

DEFAULT_FL_STUDIO_USER_DATA_DIR = os.path.expanduser(
    "~/Documents/Image-Line/FL Studio"
)
FL_STUDIO_USER_DATA_DIR = os.path.abspath(
    os.path.expanduser(
        os.environ.get("FL_STUDIO_USER_DATA_DIR", "").strip()
        or DEFAULT_FL_STUDIO_USER_DATA_DIR
    )
)
HARDWARE_DIR = os.path.join(FL_STUDIO_USER_DATA_DIR, "Settings", "Hardware")
SCRIPT_DIR = os.path.join(HARDWARE_DIR, "Universal Bridge")
SCRIPT_PATH = os.path.join(SCRIPT_DIR, "device_UniversalBridge.py")
...
def expected_bridge_deployment() -> tuple[bytes, str]:
    """Return the exact installed bytes and source hash for this checkout."""
    source_path = os.path.join(ROOT, "bridge", "device_UniversalBridge.py")
...
def check_script_installed():
    ...
    try:
        ...
        fail(f"Could not verify the installed bridge: {exc}",
             "Restore bridge/device_UniversalBridge.py, then run ./scripts/install.sh.")
        return
    if actual == expected:
        ok("Installed bridge matches this checkout", f"source SHA-256 {digest}")
    else:
        warn("Installed bridge differs from this checkout's stamped bridge.",
             "Run ./scripts/install.sh to update it, then reload the "
...
def check_bridge_live():
    ...
    try:
        ...
        fail(f"Cannot import the MCP package: {e}",
             "Run ./scripts/install.sh to build the virtualenv.")
        return
    ...
    if reported_digest:
        try:
            ...
            if reported_digest == expected_digest:
                ok("Running bridge matches this checkout",
                   f"source SHA-256 {expected_digest}")
            else:
                fail(
                    "Running FL Studio has a stale bridge loaded.",
                    "Run ./scripts/install.sh, then reload the script in FL Studio "
    ...
    if connection.verified_writes_enabled:
        ...
        fail(
            ...
            "verified write mode.",
            "Run ./scripts/install.sh and reload the script in FL Studio "
            "(View > Script output > Reload script).",
        )
        return
    try:
    ...
    try:
        ...
        fail(
            ...
            "Run ./scripts/install.sh, reload the FL bridge, then retry.",
        )
